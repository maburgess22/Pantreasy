...
export default function CustomSelect({ value, options, onChange, placeholder = "Select...", className = "" }: { value: string, options: {label: string, value: string}[], onChange: (val: string) => void, placeholder?: string, className?: string }) {
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-black/50 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4 transition-opacity" onClick={() => setIsOpen(false)}>
  ...
  return (
    <>
      ...
      {/* Portals pull the modal completely out of the parent container to the absolute top of the page */}
